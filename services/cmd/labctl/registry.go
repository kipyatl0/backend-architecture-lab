...
type Script struct {
	...
	// Несколько таблиц подряд, у каждой свои столбцы. Нужны там, где сцена
	// ведёт студента по шагам разбора: симптом, трейс, лог — это три разных
	// набора столбцов, и запихивать их в один значило бы испортить все три.
...
type Table struct {
	// Заголовок блока. Печатается над таблицей — там, где таблиц несколько и
	// студенту надо понимать, на что он сейчас смотрит.
	Title   string   `json:"title"`
